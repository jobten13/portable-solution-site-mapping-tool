...
- **2026-08-10 — Role/Label consolidation + Vendor-section role relocation (design decided, not yet built).**
  ...
  **STATUS:** Point **11** built (map→list scroll). Points **1–10** remain **DESIGN, NOT BUILT**. This effort affects Save Plan JSON, undo snapshots, GeoJSON export, PDF export text, and the print zone-summary line only in that the "Label — Role" **DISPLAY** combination (points 5+) is new formatting logic at each of those surfaces — the underlying label and role properties themselves are unchanged, so no schema migration is needed for existing saved plans.
