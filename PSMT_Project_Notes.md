...
- **2026-08-17 — Fit View removed; fit-for-print question CLOSED as answered; @page landscape adopted (commit 8c048a8, print consolidation commit 2).**
  ...
  **CONTEXT:** Commit 2 of the approved 3-commit print/export consolidation. Mid-pass, the developer reopened the Fit View question (his call, overriding the prior do-not-relitigate note). A rework was built and audited: paper-frame fit (Letter-derived constants), meters/pixel zoom math, no zoom-20 floor per developer decision (full site visibility wins over labels; zoom out only as far as site size requires), header relocation after Measure. It failed on the same fundamental as Print Fit / Print Site: one zoom cannot serve both the screen viewport and the paper frame. Landscape paper-frame fit raised zoom (wider frameW); the live map is a different aspect, so a paper-correct framing still clipped the site vertically on screen. Manual framing + Ctrl/Cmd+P demonstrably printed better. Developer visually verified (5/5).

  **DECISION:** Fit View and all fit-then-print / paper-frame auto-fit successors are **removed**. Printing is **Ctrl/Cmd+P at the user's current view** only. An **`@page` rule** (`size: letter landscape; margin: 6px 10px`) is **kept** — it fixes real UA-default margin clipping and stands alone (print dialog can still override orientation/size). Global `beforeprint`/`afterprint` hooks (scale/strip refresh, `invalidateSize`, rotate-handle clear/restore gated on print media ending + 2s fallback) and delete-handle print CSS remain. Fit-for-print framing is **CLOSED as answered, not deferred**: the screen-vs-paper mismatch is structural.

  **STATUS:** **SHIPPED** (8c048a8). Commit 3 of the consolidation (Export PDF removal / docs wrap) remains separately.
