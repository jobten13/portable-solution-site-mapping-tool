...
**Portable Solution Site Mapping Tool** (released/field-tested **v1.0**; current working build **v1.2.0-dev** on `psmt-overhaul`) — Single-file HTML app for emergency field hospital site layout. Leaflet.js map at true scale, vendor `TENT_DB` (**eight** vendors — BLU-MED, Western Shelter, DLX, ZUMRO, HDT, Craftsmen, FORTS, WillScot; **20** standard-visible models, **16** extended-hidden, **36** total), click-then-place workflow, drag/rotate handles, snap-to-face attachment, two-tier overlap (footprint red / clearance buffer amber), undo/redo (50 steps), role tagging with custom roles, debounced browser autosave with manual **Restore Autosave**, portable **Save Plan** / **Open Plan**, GeoJSON export, PDF export with map snapshot, and print fit.

**Version note:** `APP_META`, README, Quickstart, and PROJECT_MAP now read **1.2.0-dev** as of 2026-08-10. This bump reflects Tier C completion (Steps 1/3/4/5 — toasts, map-floating readout, Start Here, `#mode-display` removal; commits b1fb43b / b66ee21 / 802c1b3 / 77762fb). Prior **1.1.0-dev** (2026-07-07) covered shape-fidelity audit completion and snap phase 1; **psmt-overhaul** backlog items remain open.
...
**Capacity (#3):** Bed counts vary by hospital and acuity — user input per placement is right; optional default per tent type speeds entry. Ward/ICU roles already map to calculator consumption profiles. Facility-total arithmetic for catalog/spec bed markers: see Decisions, 2026-08-11 (BED-TALLY CONVENTION).
...
- **Map click with nothing armed gives no feedback + repeat-click placement question (needs history investigation)** — Testing surfaced that clicking the empty map while no tent is armed currently does nothing silently (`placingMode` false → early return in the map click handler) rather than showing any message. Confirmed via read-only check that the existing `NOTICE: Select a tent model first` alert inside `startPlacing()` is **UNREACHABLE** via any current UI path (tent card click and the `P` shortcut both only call `startPlacing()` when a tent is already selected) — this is pre-existing dead code, not something this session broke. **SEPARATELY:** developer states from direct memory that repeat-click placement (staying armed to place multiple of the same tent until switching tents/functions) worked in the past. Read-only check confirmed current code always disarms (`placingMode = false` via `cancelPlacing()`) after every successful placement, and confirmed this session's changes (Role/Label Steps 1/2, dialog prefixes) did not alter that behavior — but the check only covered this session's diffs, **NOT** prior git history, so it cannot confirm or rule out whether this regressed in an earlier session. Needs a git history/blame investigation, not just current-state reading, to resolve. **NOTE:** the `P` keyboard shortcut currently re-arms placement instantly without needing to reselect the tent card (since `selectedTent` stays set after `cancelPlacing()`) — this may be what was being remembered, or may be a separate, faster path worth keeping either way. Not investigated further, not fixed, not scheduled.
...
- **Utilities-exclusion disclaimer — LOCKED REQUIREMENT for the future running-totals feature** — When the running beds/sq-ft facility tally is built, it MUST carry a user-visible note that utilities (water, power, hygiene, waste) are excluded from footprint totals and must be accounted for separately. Phrase as excluded "for now" to avoid contradicting Immediate feature backlog **#5** (Utilities overlay), which remains the long-term path to actually mapping them. Logged as a requirement, not scheduled work.
- **Generator relocation out of Western Shelter grouping** — The 70 kVA Generator sits under Western Shelter because that's how it was purchased — procurement trivia, wrong for a planning tool; it serves any vendor's shelters. Relocation to an equipment/utility grouping (likely a pseudo-vendor tab or equivalent) is a UI/structure change, sequenced with the vendor-area redesign (tabs-vs-dropdown, card work) where vendor grouping is already being reshaped. Cross-reference: Structure bucket for equipment — name TBD (above). Not scheduled.
- **Vendor tabs (8) wrap** — After catalog-import Step 4 (eight vendors), the sidebar vendor-tab row wraps to **two lines** (known interim state pending the vendor-area redesign). Not fixed here; sequenced with that redesign. Not scheduled.
...
- **2026-08-10 — Role/Label consolidation + Vendor-section role relocation (design decided, not yet built).**
  ...
  **STATUS:** Points **1**, **5** (identity surfaces; hover tooltip deferred per point 7), and **11** built (Role/Label Steps 1–2). Points **2–4**, **6–10** remain **DESIGN, NOT BUILT** (point 2 is mechanical no-op until Step 3 relocates UI). Schema unchanged — `label`/`role` still separate stored properties; display formatting only.
...
- **2026-08-12 — Catalog/card redesign Step 4:** Zoom-gated on-map role text (role only; roleless/`—` = none). Knobs: `z >= 20`, min shorter-edge **48px**, text **rotates** with `angleDeg` (divIcon, centered). Hover tooltip untouched. Files: HTML, this Notes (minimal). Rollback `3fa2b7d`.
