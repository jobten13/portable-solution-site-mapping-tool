...
**Portable Solution Site Mapping Tool** (released/field-tested **v1.0**; current working build **v1.4.0-dev** on `psmt-overhaul`) — Single-file HTML app for emergency field hospital site layout. Leaflet.js map at true scale, vendor `TENT_DB` (**eight** vendors — BLU-MED, Western Shelter, DLX, ZUMRO, HDT, Craftsmen, FORTS, WillScot — plus a ninth catalog tab **Power** pseudo-vendor holding the MQ Power Generator; **20** standard-visible models, **16** extended-hidden, **36** total), click-then-place workflow, drag/rotate handles, multi-select with rigid group move/rotate/delete, snap-to-face attachment (rotation-as-intent; off for N>1 group moves), two-tier overlap (footprint red via pair-local meter-space intersection area; clearance buffer amber via layer-pixel touch-or-overlap; advisory recomputes live during drag/rotate), undo/redo (50 steps), role tagging with custom roles, debounced browser autosave with manual **Restore Autosave** (drift guard), portable **Save Plan** / **Open Plan**, GeoJSON export, and browser print (header **Print** button or Ctrl/Cmd+P). **UPDATE 2026-08-31:** current HEAD also includes zoom-derived handle padding (~40 px edge→center at all zooms), true-face snap both directions (harness `tools/snap-identity-harness.js`), door/entry marks on any face, Ramp role + hatch (Custom Size path; ramp objects not built), ramp-tier overlap routing, overlap pill cycle + pulse, placed-list focus mode, catalog ⚙ retired (Manage roles… when customs exist), and print header rework + SVG symbol legend. **UPDATE 2026-08-31 (1.5.0-dev):** working build **v1.5.0-dev**; Start Here is searchable essential help (`8a0e8a6`).

**Version note:** `APP_META`, README, Quickstart, and PROJECT_MAP now read **1.4.0-dev** as of 2026-08-25. This bump syncs operator docs after post-1.3.0 ships (#21 multi-select, search picker, overlap #28, selection underlay, and related). Prior **1.3.0-dev** (2026-08-12) covered catalog import + card redesign + Setup dissolve (`7902c85`); **1.2.0-dev** (2026-08-10) covered Tier C completion; **1.1.0-dev** (2026-07-07) covered shape-fidelity audit + snap phase 1. **UPDATE 2026-08-31:** operator-docs bump is `cf2dae9` (currency through Working queue **#21** and related); these Notes are current through HEAD `6b0e1e8` (#29–#35 and intervening ships); README/Quickstart/PROJECT_MAP were not re-synced in this Notes-only catch-up (no version bump). **UPDATE 2026-08-31 (1.5.0-dev):** `APP_META`, README, Quickstart, and PROJECT_MAP now read **1.5.0-dev**; operator-docs currency through Working queue **#3** Step A (`8a0e8a6`) and this B+C commit.
...
All calculators use per-day / per-bed math and share the same design language. Target flow: map placements → roles → bed counts → calculator inputs. **UPDATE 2026-08-31 — DEAD:** developer decision: the suite **will never be integrated** into this tool; that suite stays separate. Lead-in “planned integration at v2.5+” and Target flow above are historical only.
...
- **6.** Select-all / group move (move a whole built-out setup together). Decided 2026-06-29: bulk delete via a future select-all/group action gets a confirmation dialog ('Delete N structures?'); undo remains the backup. **SHIPPED 2026-08-25** via Working queue **#21** (`c52604d`…`ab66b03`): multi-select toggle, Ctrl/Cmd+A, rigid group move/rotate/delete, buffer-to-set, ↻ handle. WillScot Patient/Staff pair was the driving use case. **List ✕ stays per-row** (identity confirm) — not group delete.
- **7.** Mark doors/exits, optionally attached to tents where position is known. Note: touches plan-JSON schema — new geometry, NOT pure UI. Treat as additive, pre-v1 schema. **Ramp geometry (logged 2026-08-11):** ramps are directional, entrance-attached, and grade-dependent; a future ramp object likely wants grade-dependent run, not a fixed catalog footprint — see Decisions, 2026-08-11 (catalog import); not designed. **Ramp = Custom Size path — a ramp is placed today via the Custom Size workflow; closes the ramp side-note on Overhaul item 7.** **UPDATE 2026-08-31 — SHIPPED** via Working queue **#29**: Build 1 `908f6c6` (2026-08-28) Ramp preset role + placement-time ramp callout on the four hard-sided models + ramp-tier overlap routing; Build 2 `58e0a2c` (2026-08-28) any-face door/entry marks + Ramp hatch. Marker is single-host, rides drag/rotate, no buffer, no overlap participation, no enforcement. **Ramp objects NOT built** — Custom Size + Ramp role + hatch is the label. Prior “not designed” / side-note wording **SUPERSEDED** for doors/markers and for the ramp *object*; the Custom Size + Ramp role path stands.
- **8.** Labels on the structures themselves (e.g. ICU, Triage), supplementing/replacing the hover box. **Note 2026-08-24:** ≠ Working queue **#8** (Ward/ICU counting-role doctrine — closed). On-map role text shipped in redesign Step 4 / gate change #7; this Overhaul item remains the structure-labels scope as originally framed.
...
- **16.** Rotate handle and delete button sit too far from the object they control. Needs investigation. **UPDATE 2026-08-31 — DONE** via Working queue **#35** (`6b0e1e8`, 2026-08-31): zoom-derived handle padding (40 px edge→center, floor 1.0 m, no max) fixes **both halves** — handles far at high zoom, and the 28 px button overlapping/stealing drags at low zoom.
...
**Legend:** These `#1`–`#28` numbers are the **session working queue** (2026-08-18 onward). They are **distinct from** Overhaul-scope item numbers (Confirmed/Deferred above) and Immediate-feature-backlog numbers. Where names collide, cross-link explicitly — e.g. queue **#19** ≠ Overhaul deferred item **19**; queue **#8** ≠ Overhaul item **8**; queue **#17** cross-links Snap tuning backlog item **4**. **UPDATE 2026-08-31:** extended to `#35`. **UPDATE 2026-08-31 (1.5.0-dev):** extended to `#36`.

Complete historical record of `#1`–`#28`. Every row stays permanently with status, commit, and rationale; future changes **append**, never delete. **UPDATE 2026-08-31:** extended to `#35`. **UPDATE 2026-08-31 (1.5.0-dev):** extended to `#36`.

| # | Status | Record |
|---|--------|--------|
| **1** | **DONE** `b43c816` (+`d9b1e55` groundwork) | Search picker visible + dismiss (Esc consumes when picker open; outside-click; Measure-menu pattern). |
| **2** | **DONE** `8009257` | Placeholder "Address or lat, lng — press Enter to search"; replaced type-ahead idea (Nominatim rate limits made live suggestions a design project). |
| **3** | **DONE** `8a0e8a6` + this commit | Tooltip audit → hints re-advertisement → Quickstart help conventions. Sequenced **LAST**; bundles with **#18** + version bump; carries the **#12** removal build. **UPDATE 2026-08-31 — recast:** **Start Here as the help system.** Absorbs **#24** (MERGED): reference content, in-modal search, guided tour (~5 functions), keyboard/screen-reader operable; tooltips reduced to load-bearing hovers; video/voiceover quick-start **declined**. Sequenced **LAST**. Docs riders when built: #32 cog mentions, #33/#34/#35 features, Overhaul-16 status. Group-snap workaround copy (snap connector first, rejoin — Field-gated) goes here. **UPDATE 2026-08-31 — SCOPE CORRECTION + DONE:** #3 is the **single help destination** — **essential topics only**, not a comprehensive reference; developer never intended tooltip reduction — prior “tooltips reduced to load-bearing hovers” **misstated intent** (**SUPERSEDED**); tooltips/toasts/readout/aria **untouched both directions**. Guided tour **NOT built** (Field-gated). **Shipped:** Step A `8a0e8a6` — searchable Start Here (lead-in + 7 topics; keyword synonym index / settings-search pattern); Step B+C this commit — docs sweep + **1.5.0-dev** bump + Notes. |
| **4** | **DONE** `b22d782` | Placed-list two-state snap expand (chevron, 220px ↔ 60%, session-only). Deferred aria-only half resolved by **#25**. **UPDATE 2026-08-31:** **superseded by #26** focus mode (`dcabcc4`) — 60% expand path retired. |
| **5** | **DONE** `01ac9dc` | Per-object buffer edit ("Apply to Selected", undoable; `intentionalBuffer` orphan-flag fix when applied buffer is 0). |
| **6** | **DONE** `7b7ad98` | Custom bed ranges: Beds min/max optional fields; either alone = single value; both = auto-ordered range; live amber warn on inverted entry, never blocks placement ("tool delivers no matter what"); `customBeds` = `number\|{min,max}`; `hasCustomBeds` at all gates; killed silent "6-10" NaN data loss. |
| **7** | **DONE** `1abdf64` | Role labels: size gate **REMOVED**, zoom ≥ 20 + role only. Root cause: 48px absolute edge threshold (latitude-dependent knife-edge), not the min/max axis mismatch first suspected. `ROLE_LABEL_MIN_EDGE_PX` + `getPxPerFt` deleted. |
| **8** | **CLOSED by doctrine** | **≠ Overhaul item 8** (structure labels — untouched). Queue **#8** = Ward/ICU counting-role list question. Closed by doctrine: tool reports manufacturer figures under the totals caveat ⓘ; revisit only if field testing raises it. |
| **9** | **OPEN** | Manifest view, field-gated. Reframed: Plan-section manifest, printable alone via print-CSS mode switch; radio-at-print retired. |
| **10** | **DONE** `4cf923e` + `4a851c0` | Generator provenance (digest records both MQ Power sheets; TRLR70US located-source claim removed; 14.12'×6.04' kept as conservative planning footprint; developer hardware verification 56 kW, 103 gal) + MQ Power card. Accepted break: old-label generators in saved plans become uncounted. |
| **11** | **DONE** `d9380e0` | Scenario→Plan naming pass, 6 files; schema `psmt-scenario-v1` + internal identifiers deliberately kept. Commit-message header/footer erratum noted for **#18**. |
| **12** | **DONE** `19aed4d` | Was **VERIFY DONE** (confirmed-dead list for removal build riding **#18**/**#3**): NOTICE alert; `#placed-totals` id attr; `APP_META` name/lastUpdated/changelog (changelog judgment: human-read); `endCapFt` plumbing (4 sites, plan-JSON surface); `createGeoPolygon` non-octagon vestige; ellipse/circle branches — **OPEN CONFLICT** at removal time: no-backward-compat rule (2026-08-12e) vs "retained for legacy saved plans". **KEEP** (developer-confirmed): `#extended-catalog-wrap`, fidelity fields. **RECORD CORRECTIONS:** `#snap-to-selected` + Attached-to already deleted (`fd1422c`); no P-shortcut exists; `latlngToMeters` **REMOVED from the dead list** — now live via `getLayerMeterRing` (`04d0e97`). **UPDATE 2026-08-31 — DONE** `19aed4d` (2026-08-28): ellipse/circle, `endCapFt`, `createGeoPolygon` vestige, orphan removed; loader skip toast added; open conflict **resolved by the no-compat rule**; `#placed-totals` **LIVE kept**; `APP_META` meta fields **LIVE kept**. |
| **13** | **OPEN DECISION** (developer-run) | Merge strategy `psmt-overhaul` → `main`. **UPDATE 2026-08-31:** still OPEN DECISION; all work through `6b0e1e8` is pushed to `origin/psmt-overhaul`. |
| **14** | **CLOSED/rejected** | 13.6" field-hardware screenshot shows no header pressure; current chrome is the honest save-state presentation; the 2026-06-26 save-state-honesty parked question **DISSOLVES**. Revisit only if header additions recreate pressure. |
| **15** | **DONE** `789ec73` | Header Print button after Measure; plain ghost (setoff declined on evidence); `window.print()` only. |
| **16** | **CONDITIONAL** | Placement-time snap-attach; only if field-raised. |
| **17** | **OPEN** | Intermittent map blur/pan during object drag; no repro; priority rising. **Corresponds to Snap tuning backlog item 4** (cross-link; do not duplicate). Read-only observations on record from the **#28** build rider and **#21 Step 5** testing (2026-08-25): single flare during group-rotate session — self-resolved, no repro; **do not mark fixed**. Prior theory: `map.dragging.disable`/`enable` in drag paths; drag ends via map `mouseup` only; ambient snap + mid-drag `drawObject` layer rebuild. **UPDATE 2026-08-31 — hardening** `ca46375` (2026-08-26): window-level gesture end; wheel-zoom off during gestures. Three flares observed across sessions; none reproduced. Next-flare checklist: landmark slides vs soft-and-still; Zoom changed?; cursor state. **NOT marked fixed.** |
| **18** | **CONSUMED** (1.4.0-dev docs pass) | Operator-docs batch at version bump — **CONSUMED 2026-08-25** except items explicitly still open. **Shipped in pass:** snap rotation-as-intent wording; #11 erratum; beds-fields README/Quickstart; accept-vs-resolve overlap copy; PROJECT_MAP function index + workflows; getPlacedBounds/printFitToPage removed; getPlacedObjectsBounds + selection APIs documented; #snap-to-selected gone; latlngToMeters live; Ctrl/Cmd snap-suppress in README/Quickstart; Notes #21 layer; #20 wording; highlight backlog closure. **Partial #12:** `getGroupRotateHandleLatLng` removed + stale rotate-handle comment fixed (HTML). **Still open:** #12 remainder (endCapFt vestige, createGeoPolygon non-octagon, ellipse branches, etc.); **#3** in-app tooltip audit; **#24** Start Here modal copy. **UPDATE 2026-08-31:** CONSUMED commit is `cf2dae9`; remainder **folded into #3** (#12 shipped `19aed4d`; #24 MERGED into #3). |
| **19** | **STRUCK** | **≠ Overhaul deferred item 19** (zoom/scale presets + preserved formula — stays **OPEN**, untouched). Queue **#19** = comparison-tool rename — **STRUCK** (separate project). |
| **20** | **CLOSED** via **#25** side-effect | Placed-row name truncation; confirm wording at docs pass (**#18**). |
| **21** | **DONE** | Multi-select / group operations — **SHIPPED** in five steps on `psmt-overhaul`. **Step 1** `c52604d`: `selectedIds` + `primaryId` (runtime-only); Shift/Ctrl/Cmd+click toggle; Ctrl/Cmd+A; double-outline underlay; modifier mousedown does not arm drag. **Step 2** `1e04f47`: rigid group translate; one undo; snap off N>1. **Step 3** `3822995`: shared map ✕ N≥2 + count confirm; Delete/Backspace; **list ✕ per-row** (`deleteObj` + identity confirm) — overseer extrapolation rejected. **Step 4** `93f4998`: `applyBufferToSelected` on full set; one undo. **Step 5** `ab66b03`: rigid group rotate (centroid pivot); ↻ icon singleton+group; orientation-aware polar seat (`getGroupRotateHandleSeatAtRest`); dead AABB `getGroupRotateHandleLatLng` removed in 1.4.0-dev docs pass. **Step 7 (conditional) CLOSED** — facility-scale gate test passed **2026-08-25** (developer hardware: 17 structures, then 25 mixed structures; select-all, group drag + rotate; usable, no mitigation required; Step 7 never built, by design of the gate). **#21 complete end to end.** Cross-link: Overhaul item **6** closed. **UPDATE 2026-08-31:** **list ✕ per-row rationale** — developer: group delete from a list row would be “real cognitive dissonance.” **Step 5 audit history:** spawn FAIL (rotated opposite the drag); three developer-caught bugs — group ↻ never re-seats, Shift+rotate boxZoom, group-move staleness. |
| **22** | **DONE** `50642a1` | Selection exit: Esc + empty-map click deselect (suppress window kept, in commit). |
| **23** | **DONE** `90c602b` | Restore drift guard. Design **REVISED** mid-session (developer call): exact-view restore + fit-to-placed-bounds fallback **ONLY** when no placed object visible; zero-object = saved center; `fitBounds` maxZoom 18 (matches search precedent, **NOT** print-fit-derived; print-fit resurrection explicitly rejected). New helper `getPlacedObjectsBounds` (`layerBuffer\|\|layer` union) — **distinct from** removed `getPlacedBounds`; do not conflate. |
| **24** | **MERGED into #3** | Was **OPEN** — Start Here content pass; can ride **#18**. **UPDATE 2026-08-31:** **MERGED into #3** (Start Here as the help system). |
| **25** | **DONE** `1d0708c` | Placed-row cleanup: dot/name/role/✕; " — Role" dropped from row name **ONLY** (`formatObjectIdentity` untouched — confirms/readout/toasts keep it); meta removed; `title=label` rider. Closes **#20**; resolves **#4**'s aria-only deferred half. |
| **26** | **DONE** `dcabcc4` | Was **OPEN** — Expand-ratio revisit. **UPDATE 2026-08-31 — DONE** `dcabcc4` (2026-08-26): placed-list **focus mode** replaces 60% expand. |
| **27** | **DONE** `522aac9` | Was **OPEN** — Overlap pill: cycle through flagged rows + relocate under zoom. Pill is **TOP-RIGHT** under layers icon; placement plan must handle readout coexistence. **UPDATE 2026-08-31 — DONE** `522aac9` (2026-08-26): pill **cycle + pulse**; **no relocation**. |
| **28** | **DONE** `04d0e97` | Two-tier overlap fix: **(a)** footprint overlap now pair-local meter-space intersection (`getLayerMeterRing`, midpoint origin, `FOOTPRINT_MIN_INTERSECTION_AREA_M2` 0.01; same frame as snap — flush no longer false-flags); **(b)** `drawObject` mid-drag skip removed — live advisory during drag/rotate, rAF-coalesced. Buffer tier unchanged; Intentional unchanged. Spawned-subagent audit 10/10; visual pass. |
| **29** | **DONE** `908f6c6` + `58e0a2c` | Doors / Ramp path. **Build 1** `908f6c6` (2026-08-28): Ramp preset role; placement-time ramp callout on the four hard-sided models; ramp-tier overlap routing. **Build 2** `58e0a2c` (2026-08-28): any-face door/entry marks; Ramp hatch. Marker single-host, rides drag/rotate, no buffer, no overlap participation, no enforcement. **Ramp objects NOT built.** Closes Overhaul item **7**. |
| **30** | **OPEN** | Accessibility / Section 508, tiered (contrast + color-alone + aria-live; keyboard pass; map alternative incl. numeric rotation; VPAT); post-merge in spirit; settings panel born here if ever. |
| **31** | **DONE** `066e2a5` | Anchor-side true-face snap (`066e2a5`, 2026-08-28); harness `tools/snap-identity-harness.js` (6480 identity at ship). Cross-link Snap tuning backlog **6**. |
| **32** | **DONE** `1605285` | Catalog ⚙ retired (`1605285`, 2026-08-28); “Manage roles…” sentinel shown only when custom roles exist. |
| **33** | **DONE** `8b61ecf` | Print header rework (`8b61ecf`, 2026-08-28): plan, time, coords 3dp, beds, sq ft, zones, disclaimer as running text; scale ratio cut; buffer paren only when differing; Ramp excluded from Zones incl. GeoJSON `zoneSummary` + legend (Entry/Exit open-polyline chip, Ramp hatch, red/amber chips; SVG only). Legend follow-ups field-gated. |
| **34** | **DONE** `3bc0823` | Mover true-face snap (`3bc0823`, 2026-08-29); chamfers lead both directions; rect short-circuit; sticky pairing ε clamp 0.15–0.25 m; harness **6864** (+192 chamfer-led, +96 8D36, +96 GK1935). Completes Snap tuning backlog **6** with **#31**. |
| **35** | **DONE** `6b0e1e8` | Handle padding zoom-derived (`6b0e1e8`, 2026-08-31): 40 px edge→center at all zooms, floor 1.0 m, no max; three seat sites; gated `zoomend` re-seat; `touchZoom` guard. Fixes Overhaul-**16** both halves. |
| **36** | **OPEN** | Offline copy honesty pass — banner / search-tooltip / toast copy oversells offline; coordinate-jump is real (local parse) but map imagery does not load offline; reframe as failure-state warning (wording at its own plan step). Unscheduled. |

### Watch items (working queue)

- **Picker-open map click double-duty** (dismiss + deselect/select) — developer chose leave-as-is; do not build dismiss-only without new field evidence.
- **Small-object label overhang** — accepted at **#7** ship; abbreviation direction (Vbl/Ent/HC) only if field-flagged. **Replaces** the stale 48px-gate watch item (Deferred/discussed).
- **Title-hover delay** on truncated names — browser-native, accepted.
- **List-desync race** (`updateList` paints from stale `overlapState`; summary rAF updates counts/strokes, never rebuilds list; uncheck-then-count path reachable) — surfaced in the 3-vs-2 investigation (which itself was **NO BUG**: third strip below the fold); **subsumed by #28**'s live recompute — verify at next field pass.
- **Drag smoothness at facility scale** — Step-7-style mitigation designed (not built; gate closed without it).
- **Door+Snap armed together** — as designed.
- **Door-mark move requires exiting Door mode** — as designed. **UPDATE 2026-08-31 — STALE:** audit-verified `startOpeningDrag` has **no** `doorMode` gate; drag-to-adjust works **in** Door mode.
- **Off-map release drops in place** — undo recovers.
- **Shift-click empty map may zoom one step** — checklist item.
- **↻/✕ collision nudge** — deferred until a visual test shows need.
- **Unknown shape string falls through to rect** — accepted.
- **Placing-mode wheel-zoom gap** — pre-existing; not introduced by #17/#35 gesture zoom guards.
- **Armed model persists while browsing vendor tabs** — as designed.

### Field-gated

- **Group snap** — rigid-set by outer faces; designable on **#34** true-face work. Workaround (in **#3** Start Here Snap topic): snap connector piece first, then rejoin.
- **Print-legend follow-ups** — field-gated after **#33**.
- **Guided tour** — closed-modal spotlight design (consumer-standard pattern); Esc/teardown contract to be designed if field evidence calls for it. Separate from **#3** (tour not built in Step A).
...
| Two-tier overlap | 0.8.7 | Footprint (red) vs buffer (amber); **Intentional** checkbox; overlap pill; layer-space geometry at ship. **UPDATE 2026-08-24:** footprint tier now pair-local meter-space (`04d0e97` / Working queue #28); buffer remains layer-pixel. |
| Fresh open on load | 0.8.7 | No auto-restore; **Restore Autosave** is explicit |
| ZUMRO Interconnect | 0.8.5 | 7.17'×6.92' in `TENT_DB` |
| Snap to face | 0.8.3–0.8.4 | Four-face attachment; rotation match (checkbox-era); auto-uncheck. **UPDATE 2026-08-24:** ambient drag-snap uses rotation-as-intent (0/90 relative to anchor), not forced co-rotation (`a2f1ee8`). |
...
| Notes hash insert (print commit 3) | de21c41 | Insert `14c85c1` into Notes Last-updated line |
| Catalog beds unit label | 940966c | Add "beds" unit to catalog card bed-count display |
| Selection footprint highlight | 5b6bc44 | Double-outline underlay for selected footprint (`ensureSelectionUnderlay`); closes Overhaul item 18 highlight half / Immediate #2 highlight |
| Remove dead placement-attach | fd1422c | Delete `#snap-to-selected` checkbox + Attached-to path |
| Rotation-as-intent drag-snap | a2f1ee8 | Snap uses pre-drag intent angle quantized to nearest 0/90 relative to anchor — not forced co-rotation |
| Measure strip centering | 4f7a9e5 | Center `#measure-strip` under Measure button |
| Measure labels off geometry | cd8175e | Distance/area tooltips offset off the measured geometry |
| Search picker visible | d9b1e55 | Multi-match `#search-results` fixed/reparented (Working queue #1 groundwork) |
| Search picker dismiss | b43c816 | Esc + outside-click dismiss (Working queue #1) |
| Search Enter placeholder | 8009257 | Placeholder hints Enter-to-search (Working queue #2) |
| Scenario → Plan naming | d9380e0 | User-facing Scenario→Plan; schema `psmt-scenario-v1` kept (Working queue #11) |
| Generator provenance | 4cf923e | Digest/OEM correction; conservative footprint retained (Working queue #10) |
| MQ Power card identity | 4a851c0 | Two-line generator card + placed identity (Working queue #10) |
| Placed-list snap expand | b22d782 | Two-state 220px ↔ 60% expand (Working queue #4) |
| Apply buffer to Selected | 01ac9dc | Per-object buffer edit + intentionalBuffer orphan-flag fix (Working queue #5) |
| Selection exit | 50642a1 | Esc + empty-map click deselect (Working queue #22) |
| Restore Autosave drift guard | 90c602b | Exact-view restore + `getPlacedObjectsBounds` fit fallback (Working queue #23) |
| Role labels zoom+role only | 1abdf64 | Remove 48px min-edge gate (Working queue #7) |
| Custom Size bed ranges | 7b7ad98 | Beds min/max; `customBeds` number\|{min,max} (Working queue #6) |
| Header Print button | 789ec73 | `#btn-print` after Measure (Working queue #15) |
| Placed-list row cleanup | 1d0708c | Dot/name/role/✕; closes truncation Working queue #20/#25 |
| Two-tier overlap #28 | 04d0e97 | Meter-space footprint intersection + live advisory during drag/rotate (Working queue #28) |
| Multi-select Step 1 | c52604d | Selection set + Shift/Ctrl/Cmd toggle + Ctrl/Cmd+A + underlay (Working queue #21) |
| Rigid group move | 1e04f47 | Group translate; snap off N>1 (Working queue #21 Step 2) |
| Group delete | 3822995 | Shared map ✕; Delete/Backspace; list ✕ per-row (Working queue #21 Step 3) |
| Buffer to selection set | 93f4998 | Apply to Selected on all members; one undo (Working queue #21 Step 4) |
| Group rotate + ↻ handle | ab66b03 | Centroid pivot; orientation-aware seat; icon swap (Working queue #21 Step 5) |
| Notes catch-up through 04d0e97 | `145527f` | Full Working queue `#1`–`#28` record + shipped/Deferred/Architecture sync (2026-08-24) |
| Operator docs 1.4.0-dev | `cf2dae9` | README/Quickstart/PROJECT_MAP/Notes + APP_META bump through #21; partial #12 dead-code slice (2026-08-25) |
| #17 gesture-end / mid-gesture zoom harden | `ca46375` | Window-level gesture end; wheel-zoom off during gestures (2026-08-26) |
| Placed-list focus mode | `dcabcc4` | Replaces 60% expand (Working queue #26; 2026-08-26) |
| Overlap pill cycle + pulse | `522aac9` | Cycle navigation + appearance pulse; no relocate (Working queue #27; 2026-08-26) |
| Ramp role + callout + overlap tier | `908f6c6` | #29 Build 1 (2026-08-28) |
| Door/entry marks + Ramp hatch | `58e0a2c` | #29 Build 2 (2026-08-28) |
| True-face snap (anchor) + harness | `066e2a5` | Chamfers/facets/arm faces; `tools/snap-identity-harness.js` 6480 (Working queue #31; 2026-08-28) |
| Catalog ⚙ retired | `1605285` | Manage roles… when customs exist (Working queue #32; 2026-08-28) |
| Print header + symbol legend | `8b61ecf` | Print strip rework + SVG legend (Working queue #33; 2026-08-28) |
| Dead-code removal + loader skip | `19aed4d` | #12 remainder; ellipse/circle/`endCapFt`/vestige gone; loader skip toast (2026-08-28) |
| Mover true-face snap | `3bc0823` | Chamfers lead both directions; harness 6864 (Working queue #34; 2026-08-29) |
| Handle offset follows zoom | `6b0e1e8` | 40 px edge→center; Overhaul-16 both halves (Working queue #35; 2026-08-31) |
| Start Here searchable help (Step A) | `8a0e8a6` | Lead-in + 7 topics; keyword synonym index; modal search (Working queue #3; 2026-08-31) |
| Operator docs 1.5.0-dev (Step B+C) | this commit | README/Quickstart/PROJECT_MAP/Notes + APP_META bump through #3; #36 logged (2026-08-31) |
...
6. **True-face snapping for irregular shapes** — GK1935 **elongated-octagon vs cut-corner-rectangle** naming question **RESOLVED** (shape type renamed to `cut-corner-rectangle`; geometry unchanged). Remaining work: implement true-face snap (backlog **#6**) for irregular shapes using certified footprints — bbox snap remains interim for GK1935, GK20, cut-corner-square, plus, etc. (`ellipse` legacy plans only). **UPDATE 2026-08-31 — DONE** via Working queue **#31** (`066e2a5`, 2026-08-28, anchor-side) + **#34** (`3bc0823`, 2026-08-29, mover; chamfers lead both directions). Harness `tools/snap-identity-harness.js` tracked (6480 → 6864).
7. **How-to / help text still references the now-hidden #snap-to-selected checkbox** — fold this doc-sync into the next snap commit. **SUPERSEDED 2026-08-24:** `#snap-to-selected` deleted (`fd1422c`); sidebar How-to already removed in Tier C (`802c1b3`). Closed as obsolete.
...
Permanent record. Rows are retained and marked **DONE** / **SUPERSEDED** / **CLOSED** rather than removed when work ships or is rejected.

| # | Item | Notes |
|---|------|-------|
| 1 | **Tablet mode / sidebar hide** | **CLOSED/rejected** (2026-08-12 redesign close-out). Tablet target dropped; sidebar is a dense precision UI; walk-the-site served by browser print. Revisit only as possible view-only mode if field testing demands it. |
| 2 | **Click placed row → fly-to + highlight** | Pulse map object; brief toast. Navigation half largely shipped (list↔map); **highlight half** still open — priority rationale strengthened by field-observed confusion locating objects in dense layouts. **UPDATE 2026-08-24:** both halves **CLOSED** — highlight via double-outline underlay **SHIPPED** `5b6bc44`. Optional pulse/toast not built — not required to close. |
| 3 | **Capacity tracking** | **SHIPPED:** Placed running totals; All / Ward·ICU bed toggle (exact match; Triage excluded pending clinical input); Custom Size optional beds; manufacturer-spec bed sources + totals caveat ⓘ on the Placed header. **OPEN (clinical):** whether Triage counts as bed space; whether the Ward/ICU exact-match list needs broadening (e.g. Pre-Op / Post-Op). See Decisions 2026-08-11 (BED-TALLY CONVENTION) and 2026-08-12 redesign COMPLETE (d). **UPDATE 2026-08-24:** clinical questions **CLOSED by doctrine** (Working queue **#8**) — manufacturer figures under the totals caveat; revisit only if field testing raises it. |
| 4 | **Access corridors** | Ambulance, staff, evacuation paths between structures |
| 5 | **Utilities overlay** | **CLOSED/rejected** (2026-08-12 redesign close-out). Utilities are load-dependent calculated quantities, not fixed-footprint objects; Water/Power calculators must answer first; likely never this tool's map-overlay scope. Calculator-suite integration (v2.5+) remains the path — at suite level, not as a map overlay. **UPDATE 2026-08-31:** calculator-suite integration **DEAD** — suite stays separate; never integrated into this tool. |
...
| — | **Selected object footprint highlight** | **SUPERSEDED / SHIPPED 2026-08-24** (`5b6bc44`): double-outline selection underlay (`ensureSelectionUnderlay`) on the selected footprint. Row retained for history. |

### Design notes (backlog)

**Capacity (#3):** Core facility tally **SHIPPED** (running totals, Ward/ICU toggle, Custom beds, manufacturer-spec sources + totals caveat ⓘ). Ward/ICU counting-role breadth (Working queue **#8**) **CLOSED by doctrine 2026-08-24** — manufacturer figures under the totals caveat; revisit only if field testing raises it. Facility-total arithmetic: Decisions 2026-08-11 (BED-TALLY CONVENTION).

**Utilities (#5):** **CLOSED/rejected** as a map overlay (2026-08-12). Path forward is calculator-suite integration (v2.5+): Water/Power calcs answer load-dependent quantities; map capacity totals feed the suite — not a utilities drawing layer on the map. **UPDATE 2026-08-31:** that path is **DEAD** — suite stays separate.
...
Five calculators in a tabbed shell. Shared deployment header (days/beds/buffer once → all tabs). No map. Standalone logistics tool for teams that do not need spatial layout. **UPDATE 2026-08-31:** calculator-suite integration into PSMT is **DEAD**; this shell (if built) stays a **separate** product.

### v2.5 — PSMT + calcs combined (non-PWA)

Map and calculators in one shell. Beds/roles (and eventually capacity) on the map propagate to calc tabs. Multi-file architecture; no install requirement. **Key integration milestone.** **UPDATE 2026-08-31 — DEAD:** will never be built as an integrated product; suite stays separate.
...
**UPDATE 2026-08-31:** flow above is historical only (integration DEAD).

### v3 — Unified PWA

v2.5 packaged as installable offline PWA. Tile caching for expected deployment areas. Multi-file + service worker required at this scale. **UPDATE 2026-08-31:** v3-as-combined-app is **DEAD** with v2.5; PSMT PWA (if ever) is standalone.

---

## Architecture notes

**Overlap geometry (v0.8.7+; hybrid as of `04d0e97` / Working queue #28):** **Footprint tier** — pair-local meter-space intersection area (`getLayerMeterRing` + midpoint origin via `latlngToMeters`; `FOOTPRINT_MIN_INTERSECTION_AREA_M2` 0.01) — same frame snap places in. **Buffer tier** — Leaflet layer-pixel rings + touch-or-overlap (unchanged). Advisory recomputes live during drag/rotate (`scheduleOverlapSummary` from `drawObject`, rAF-coalesced). Supports rects, plus hubs, cut-corner rectangle, ellipse, etc. Triangulation-failure fallback still over-warns (touch-or-overlap) rather than inventing area. **UPDATE 2026-08-31:** ellipse/circle branches **removed** in Working queue **#12** (`19aed4d`, 2026-08-28); no legacy-plan compat (2026-08-12e).
...
**Single-file vs multi-file:** PSMT stays single-file for distribution until v2.5+. Combined app should be multi-file PWA with service worker. **UPDATE 2026-08-31:** calculator-suite integration **DEAD** — “until v2.5+ combined app” is historical; PSMT remains single-file by product choice (no suite merge).
...
- **Collapsible placed list** — **SUPERSEDED** by **Placed-list two-state snap expand** (Deferred/discussed, 2026-08-12 redesign close-out). Do not design a free-drag height slider. **UPDATE 2026-08-31:** 60% expand itself **superseded by #26** focus mode (`dcabcc4`, 2026-08-26).
...
- **Search multi-match picker** — a change to show a placeholder picker dropdown for multi-result searches (instead of auto-flying to result[0]) was built, audited clean, then REVERTED: the picker is invisible because `#search-results` (a `<select>` positioned top:100% below the search bar) is CLIPPED by `#header overflow-y:hidden` (pre-existing bug since ~v0.7.7). Multi-match needs the picker, so it's BLOCKED until the clipping is fixed (reposition like the Measure menu's position:fixed, or replace the native select). Related search findings: facility-name searches (e.g. "UC Davis Medical Center") land on the building centroid — a better future fix is fit-to-bounding-box, not centroid; coordinate-jump works offline; no tile caching offline. **SUPERSEDED 2026-08-24:** picker **SHIPPED** (`d9b1e55` + `b43c816`, Working queue #1) — fixed positioning + body reparent + Esc/outside-click dismiss. Centroid/fit-bbox findings stand as future work.
...
- **Placed-list two-state snap expand** — Chevron on the Placed header toggling default **220px** ↔ expanded (~**60%** of sidebar) for post-build refinement. **ABSORBS/supersedes** the old "Collapsible placed list" entry. Free-drag height slider considered and **rejected** (precision-drag failure modes for stressed users). Not designed, not scheduled. **SUPERSEDED 2026-08-24:** **SHIPPED** `b22d782` (Working queue #4). Expand-ratio revisit is Working queue **#26**.
- **Card display labeling pass** — Catalog cards show bed numbers with no unit label (e.g. `20'×32.5' · 10` — 10 of what?); Custom objects' placed-row shows Role positioned like Vendor, unlabeled. One future pass covers both. Not scheduled. **UPDATE 2026-08-24:** catalog **"beds" unit label SHIPPED** (`940966c`). Role-unlabeled / remaining labeling half still open if wanted.
- **Buffer-edit gap** — No visible way to add/edit a clearance buffer on an already-placed object. Logged as a bug/needed fix. Not scheduled. **SUPERSEDED 2026-08-24:** **SHIPPED** `01ac9dc` — "Apply to Selected" (Working queue #5).
...
- **Custom beds is single-value** — Catalog-style ranges ("6 to 10") are not supported in the Custom Size beds field; revisit if field use demands (additive). Not scheduled. **SUPERSEDED 2026-08-24:** **SHIPPED** `7b7ad98` — Beds min/max optional fields; `customBeds` = number|{min,max} (Working queue #6).
- **Role-label 48px min-edge gate** — Gate measures the **shorter** footprint edge, but on-map role text now runs along the **long** axis (Step 4 amendment); may hide labels on narrow-but-long objects where text would fit lengthwise. Watch item from Step 4. Not scheduled. **SUPERSEDED 2026-08-24:** gate **REMOVED** (`1abdf64`, Working queue #7) — zoom ≥ 20 + role only. Replaced by Working-queue watch item **small-object label overhang** (accepted).
...
- **2026-08-17 — Fit View removed; fit-for-print question CLOSED as answered; @page landscape adopted (commit 8c048a8, print consolidation commit 2).**
  ...
  **STATUS:** Commit 3 COMPLETE (14c85c1).
...
- **2026-06-28 — Snap direction revised to ambient drag-snap (leading candidate); market/analog survey; custom-object handling; survey-first rule.**
  ...
  **DECISION A — Snap direction revised to user-toggled AMBIENT DRAG-SNAP (leading candidate, not yet locked):** Instead of the Measure-style dropdown with explicit pick-anchor/pick-mover/commit, the leading direction is the consumer-standard ambient model:
  ...
  - **UPDATE 2026-08-24:** shipped ambient snap does **not** force co-rotation — rotation-as-intent (`a2f1ee8`) quantizes the mover's pre-drag angle to nearest 0/90 relative to the anchor. Live alignment guide was never built in phase 1 (still aspirational / snap-tuning territory).
...
- **2026-06-26 — Snap feature unified and folded into header rework (#11/#15).**
  ...
  **DECISION 3 — DEFERRED, open question for the header work: save-state indicator honesty.** When the header save-state chrome (currently "Restore Autosave" button + "Autosaved <timestamp>" text) is regrouped/collapsed during #11/#15, the labeling must not overclaim — concern on record that a bare "Saved" label could mislead (autosave writes to localStorage only and is weaker than Save Plan; and a debounced write is pending for up to 500ms after an edit). NOT decided — parked to be settled when the header is designed, because it dovetails with the save-state grouping. Reference: `SESSION_DEBOUNCE_MS = 500` (line 561). **UPDATE 2026-08-24:** Working queue **#14** **CLOSED/rejected** — 13.6" field-hardware screenshot shows no header pressure; current chrome is the honest save-state presentation; this parked question **DISSOLVES**. Revisit only if header additions recreate pressure.
...
- **2026-08-08 — Tier C — header/sidebar/mode-display UX: decided and locked.**
  ...
  **PARKED ITEMS** (not part of Tier C, logged for later, not scheduled):
  - Legacy `#snap-to-selected` checkbox (line ~427) and its associated dead "Attached to X (face)" message (map-click handler, ~line 2061) — confirmed inert this session. Eventual removal intent (checkbox, hint text, and the dead message code path) noted per developer recollection but **NOT scheduled**. **UPDATE 2026-08-24:** **DELETED** (`fd1422c`). Working queue #12 records this as a confirmed-dead correction (already gone before the removal build).
...
- **2026-08-10 — Tier C COMPLETE — Step 5 shipped (77762fb); full header/sidebar/mode-display rework closed out.**
  ...
  One medium audit finding, resolved by decision not code: the legacy "Attached to X (face)" message (dead code behind the already-inert #snap-to-selected checkbox) now has no UI surface if ever reached. Developer decided to leave as-is rather than patch — will be addressed in an upcoming dead-code sweep instead. **UPDATE 2026-08-24:** removed in `fd1422c`.
  ...
  **UPDATE 2026-08-24:** **SHIPPED** `50642a1` (Working queue #22) — Esc terminal arm + empty-map click deselect via `clearSelection()`; suppress window kept.
...
- **2026-08-10 — Role/Label consolidation + Vendor-section role relocation (design decided, not yet built).**
  ...
  **FINAL DECISION** — label and role remain two separate stored properties, **UNCHANGED IN MECHANISM**, but role becomes the sole user-facing customization point:
  ...
  5. **IDENTITY TEXT SHOWN AS "LABEL — ROLE"** (combined), everywhere the app currently shows label alone, **WHEN** role is set to something other than "—": delete confirmations, "Selected X — drag to move" readout, snap toasts, hover tooltip, placed-list row, and (until removed with Export PDF in print consolidation commit 3) PDF object lines. Example: "Delete BLU-MED 2032.5 — Triage?" instead of just "Delete BLU-MED 2032.5?". When role is still "—", these surfaces show label alone (unchanged from today). This resolves the earlier concern about role-only messages being ambiguous across multiple same-role objects (e.g. "Delete Triage?" not knowing which Triage tent) — vendor/model in the combined string always disambiguates which physical object. **SHIPPED (Step 2) for identity surfaces via `formatObjectIdentity`:** delete confirms (map ✕ / Shift+click / list), Selected readout (map + list), snap toast, placed-list name, delete/role `aria-label`s; PDF object lines (role segment conditional) shipped then later removed with Export PDF. **Hover tooltip intentionally unchanged** this step (point 7 — kept as bold label + separate Role line). Legacy "Attached to…" path untouched (dead-code sweep). Role UI still in Label & Style (Step 3 relocates). **UPDATE 2026-08-24 (Working queue #25 / `1d0708c`):** placed-list **row name** is label-only (role lives in the row dropdown); `formatObjectIdentity` **untouched** — confirms, Selected readout, and toasts still use "Label — Role". **UPDATE 2026-08-25:** Shift+click delete removed (`c52604d`, Working queue #21 Step 1); delete paths are now map ✕ (group-aware), list ✕ (per-row), and Delete/Backspace key.
...
- **2026-08-12 — Catalog/card redesign Step 4:** Zoom-gated on-map role text (role only; roleless/`—` = none). Knobs: `z >= 20`, min shorter-edge **48px**, text **rotates** with `angleDeg` and runs along the object's **long axis** when length > width (divIcon, centered). Hover tooltip untouched. Files: HTML, this Notes (minimal). Rollback `3fa2b7d`. **UPDATE 2026-08-24:** 48px min-edge gate **REMOVED** (`1abdf64`, Working queue #7) — zoom ≥ 20 + role only.
...
- **2026-08-12 — Catalog/card redesign COMPLETE + session decisions.**
  ...
  **a. Steps 3–5b shipped this session (summary):**
  ...
  - **Step 4** (`8bc0549` + long-axis Notes `c39083a`): zoom-gated on-map role text — `z >= 20`, min shorter-edge **48px**, text rotates with footprint and runs along the **long axis** when length > width; roleless/`—` = none; hover tooltip untouched. **UPDATE 2026-08-24:** 48px gate removed (`1abdf64`).
  ...
  - **Step 5b-2** (`3e4dedd`): Custom Size optional single-value beds; additive `customBeds` (plan/undo/autosave) / GeoJSON `custom_beds` when set; blank = uncounted; typed incl. 0 = bed source. **UPDATE 2026-08-24:** ranges via `7b7ad98` (Working queue #6).
  ...
  **b. Backlog #5 (Utilities overlay) — CLOSED/rejected.** Utilities are load-dependent calculated quantities, not fixed-footprint objects (a 25-bed hospital might need 2 or 10 water bladders). The estimator suite's Water and Power calculators must answer those questions first; a map overlay is likely never this tool's scope. The utilities disclaimer's **"for now"** wording was **removed** as a consequence (**SUPERSEDES** the earlier locked "for now" phrasing requirement). Calculator-suite integration (**v2.5+**) remains the path for utilities logistics — at suite level, not as a map overlay. **UPDATE 2026-08-31:** calculator-suite integration **DEAD** — suite stays separate.

  **c. Tablet target — DROPPED.** Feature backlog **#1** (Tablet mode / sidebar hide) **CLOSED/rejected.** The sidebar is now a dense precision-target interface unsuited to touch; the walk-the-site use case is served by **browser print**. Only revisit path: a possible view-only mode if field testing demands it. Roadmap wording trimmed (desktop install; tablet UX removed from the v1 gap list).

  **d. Ward/ICU counting-role list — flagged for clinical input.** Whether Triage counts as bed space, and whether the exact-match list needs broadening (Pre-Op / Post-Op, etc.) — resolve with field testing + clinical feedback. Cross-reference Overhaul item **9**'s existing clinical-input note (bed counts / ward↔ICU conversion). **UPDATE 2026-08-24:** Working queue **#8** **CLOSED by doctrine** — tool reports manufacturer figures under the totals caveat; revisit only if field testing raises it. (**≠** Overhaul item 8 structure labels.)
...
- **2026-08-24 — Intentional checkbox KEPT (accept vs resolve).**

  Per-tent buffer edit (Working queue **#5**, `01ac9dc` "Apply to Selected") is **"resolve"** — change the clearance value. The **Intentional** checkbox is **"accept"** — keep the buffer conflict but omit that structure from amber advisories. Removing Intentional would push users to falsify clearance data to silence the advisory. **Decision:** keep Intentional. Quickstart (and related operator copy) should distinguish accept vs resolve — rides Working queue **#18** docs batch.

- **2026-08-24 — Working-queue closures without build: #8 doctrine, #14 rejected, #19 struck.**

  - **Queue #8 CLOSED by doctrine** (≠ Overhaul item **8** structure labels): Ward/ICU counting-role list question closed — tool reports manufacturer figures under the totals caveat ⓘ; revisit only if field testing raises it.
  - **Queue #14 CLOSED/rejected:** 13.6" field-hardware screenshot shows no header pressure; current chrome is the honest save-state presentation; the 2026-06-26 save-state-honesty parked question **DISSOLVES**. Revisit only if header additions recreate pressure.
  - **Queue #19 STRUCK** (≠ Overhaul deferred item **19** scale presets — that item stays **OPEN** with preserved formula): queue #19 was a comparison-tool rename on a separate project — struck from this queue.

- **2026-08-24 — Working queue #28 fix rationale (two-tier overlap).**

  Flush false-positive + drag blindness. **(a)** Option **A2** chosen: footprint intersection in pair-local **meter-space** (`getLayerMeterRing`, midpoint origin via `latlngToMeters`, `FOOTPRINT_MIN_INTERSECTION_AREA_M2` 0.01) — same frame snap places in — over minimal **A1** strict-clip, on a fidelity-first steer (feet-and-inches planning; frame consistency with snap is the structurally correct fix). Buffer tier and Intentional untouched. **(b)** Option **B1**: remove `drawObject` mid-drag skip so `scheduleOverlapSummary` always runs (rAF-coalesced) — simplest correct path; covers rotate as well. Commit `04d0e97`. Spawned-subagent audit 10/10; developer visual pass.

- **2026-08-24 — Notes catch-up (this entry).**

  `PSMT_Project_Notes.md` catch-up through `04d0e97`: Working queue (2026-08-18 numbering) recorded as complete historical `#1`–`#28` table; shipped table extended for post-`14c85c1` commits; stale Deferred/Immediate/Overhaul/Architecture statements SUPERSEDED or UPDATED in place; historical Decisions annotated append-only (not deleted). Operator README/Quickstart/`PROJECT_MAP` currency remains Working queue **#18** (version-bump batch).

- **2026-08-31 (recorded) — Doors doctrine** (shipped with #29 Builds 1–2, `908f6c6` / `58e0a2c`, 2026-08-28).

  Planner-asserted deployment record — same class as roles. No door types, no door buffers, no direction. Any face on any shape, “or the feature is useless.” Callout-not-blocker, with jurisdiction rationale. Marker weight vs Ramp (hatch is Ramp’s label). Single-host marker; rides drag/rotate; no buffer; no overlap participation; no enforcement.

- **2026-08-31 (recorded) — Ramp = Custom Size + Ramp role** (shipped with #29).

  Hatch is its label; zero beds; **ramp objects declined**. Closes Overhaul item 7’s ramp-object side-note without a dedicated catalog ramp footprint.

- **2026-08-31 (recorded) — Entry/Exit label.**

  Four readers; plain register. Ingress/egress rejected. Hallway seam covered by mark placement (not a separate seam type).

- **2026-08-31 (recorded) — 2D plan view is the medium.**

  No z-order / elevation, ever.

- **2026-08-31 (recorded) — Loader “failures speak”** (shipped with #12 remainder `19aed4d`, 2026-08-28).

  Skip/notice toast when loader cannot honor a plan field; no silent drop that pretends success.

- **2026-08-31 (recorded) — #34 testing lesson** (shipped `3bc0823`, 2026-08-29).

  Test **both directions** of asymmetric gestures (anchor-side vs mover-side; chamfers lead both ways).

- **2026-08-31 (recorded) — Never Z an open outline.**

  Standing geometry/print rule: do not close an open polyline with a Z (or equivalent) that invents a false edge.

- **2026-08-31 (recorded) — Overhaul-16 closure** (shipped #35 `6b0e1e8`, 2026-08-31).

  Zoom-derived handle padding closes both halves (far at high zoom; blocking at low zoom).

- **2026-08-31 (recorded) — Geometry audits trace outputs on paper.**

  Auditor/developer practice: paper-trace harness/geometry outputs; do not trust screen-only checks for seat/snap math.

- **2026-08-31 (recorded) — Per-section catalog chevrons.**

  Considered; **declined** pending field exposure to placed-list focus mode (#26).

- **2026-08-31 (recorded) — AI auto-layout.**

  Considered as a thought experiment; **declined**.

- **2026-08-31 (recorded) — Video/voiceover quick-start.**

  **Declined**; Start Here / #3 help system remains text-first.

- **2026-08-31 (recorded) — Calculator-suite integration DEAD.**

  Developer: the suite **will never be integrated** into this tool; that suite stays separate. Annotates overview, roadmap v2/v2.5/v3, integration data flow, Architecture single-file note, Immediate backlog #5, Design notes Utilities, Deferred utilities disclaimer, and redesign close-out **b**.

- **2026-08-31 (recorded) — Notes catch-up through `6b0e1e8`.**

  Docs-only pass (this file): Working queue `#29`–`#35`; annotations on #3/#4/#12/#13/#17/#18/#21/#24/#26/#27; Field-gated + Watch items; Overhaul 7/16; Snap tuning 4/6; Decisions + Process record; shipped table rows; §4 stale-spot UPDATEs. No README/Quickstart/PROJECT_MAP; no version bump. Pattern of `145527f`.

- **2026-08-31 (recorded) — Start Here help (#3) — search kept for cheap field-gated topic growth.**

  In-modal search retained so new topics can land without UI redesign; keyword synonym index follows settings-search / emoji-picker precedent (survey-first satisfied).

- **2026-08-31 (recorded) — Help carries no troubleshooting content.**

  Genre rule: Start Here is essential get-going topics only — not a comprehensive reference and not a troubleshooter.

- **2026-08-31 (recorded) — “Above grade” register call.**

  Ramp purpose copy uses “above grade”; ground level / elevated ride in search keywords (user vocabulary).

- **2026-08-31 (recorded) — Copy lessons for help content.**

  Purpose-first feature bullets; no completeness claims (“every symbol”) as stale-magnets.

- **2026-08-31 (recorded) — A-then-B+C packaging; 1.5.0-dev as operator-docs carrier.**

  Step A modal (`8a0e8a6`) then B+C docs/bump/Notes (this commit). Version **1.5.0-dev** carries README/Quickstart/PROJECT_MAP sync.

- **2026-08-31 (recorded) — #3 tooltip-reduction wording was a misstatement.**

  Developer never intended tooltip reduction; tooltips/toasts/readout/aria stay untouched both directions. Prior recast phrase **SUPERSEDED** by #3 DONE row UPDATE.

---

## Process record

- **2026-08-31 (recorded) — Request/report protocol by type** (developer’s 2026-08-25 protocol; standing for recon / build / audit).

  Developer (verbatim):

  > What I send and what comes back, by request type:
  > Read-only recon/investigation. I send: numbered questions about
  > specific code behavior, with the context needed to answer them
  > (approximate line numbers from prior reports, the observed
  > symptom, hypotheses to check), and an explicit no-changes header.
  > Cursor returns: findings per question with current line
  > references, handler orders, state variables, and flags — no code
  > edits.
  > Build. I send: the approved design (so Cursor can't drift from
  > decisions you made), the specific changes numbered, explicit
  > DO-NOTs fencing the blast radius, and a REPORT-BACK list. Cursor
  > returns: a diff summary per numbered change, plus any deviations
  > with reasoning.
  > On 'diff' specifically: I never see the diff itself. Cursor works
  > with the full file and the actual working-tree diff against HEAD;
  > I work only with what it reports about that diff. When a report
  > says '+30/−8, 7 hunks,' that's Cursor describing git diff output.
  > I use those counts as an integrity check.
  > Audits (/auditor). I send: the audit standard — the approved
  > design restated in full, so the auditor checks the diff against
  > what you approved, not against what the coder says it built —
  > plus specific verification points and known risk spots. It
  > returns: PASS/FAIL per point with line references, findings only.
  > The structural point: you and Cursor see the code; I hold the
  > decisions, the history, and the checks.

  Standing rules (same layer):

  - Plan-first outside standing plans.
  - Reports are **diff summaries**, not code dumps; raw `git status` + `git diff --stat` first.
  - Build completely for the authorized scope; plain English.
  - Commit format: developer-authored messages; **no Co-authored-by**.
  - Audit tiers + Step 0 tree check; **no Coder audits** (audits run from a separate audit chat only).
  - Tab modes respected (Ask = read-only; Agent only on AUTHORIZED EXECUTE).
  - Copy boxes for every Cursor instruction; READ-ONLY vs AUTHORIZED stated explicitly.
  - Files are created only when the instruction explicitly names the path; long reports go to a file only on instruction (2026-08-31 incident).
  - Every Overseer instruction opens with its mode header (`AGENT — AUTHORIZED TO EDIT` / `ASK — READ-ONLY`; audits: `WINDOW AUDIT` / `SPAWNED SUBAGENT AUDIT`).
  - Plan-first applies to every build including docs-only passes; a recon checklist is not a plan (2026-08-31, #3 B+C: unplanned docs sweep took two audit FAIL rounds to reach currency).
