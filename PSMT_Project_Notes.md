...
- **2026-08-11 — Catalog import from PSMT Data Transfer.json: doctrine and locked decisions (Step 0).**
  ...
  **2. IMPORT SCOPE.** All **32** transfer products enter the data layer, tier-tagged. **STANDARD** (visible) = the current **16** `TENT_DB` models + **4** hard-sided newcomers (Craftsmen 8-Bed ICU Trailer, FORTS Model 38, WillScot Patient Unit, WillScot Staff Unit). **EXTENDED** (hidden) = **16** transfer-only soft-sided products. Architecture is flag-not-branch: a working but hidden extended-catalog toggle will be built (intentional, documented — not dead code).
  ...
  **11. FOLLOW-UP WORK LIST.** Never-audited rect models needing an eventual fidelity pass: BLU-MED 2032.5 / 2039 / Vestibule; Western Vestibule / Generator; DLX X-24 / X-32; ZUMRO 400 / 600 / Interconnect; HDT 305 — plus, when surfaced, all EXTENDED footprints. Does **not** reopen the closed non-rect shape-fidelity audit (GK20, GK1935, ZUMRO Quad, X-HUB, 8D36 remain COMPLETE).
  ...
  **STATUS:** Doctrine locked for subsequent import steps. Step 0 changes: this Decisions entry; Overhaul item 7 ramp cross-ref; Capacity #3 pointer; Last-updated bump; `.gitignore` line for `PSMT Reference Materials/`; `PSMT Data Transfer.json` staged for commit. No `TENT_DB` / digest / HTML catalog changes in this step.
