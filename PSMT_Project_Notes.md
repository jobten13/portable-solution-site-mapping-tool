...
- **Object/shape fidelity review** — audit `TENT_DB` shapes and dimensions against real-world structures. Touches the provenance-gated vendor catalog.
  ...
  **Remaining audit (example — WS GK1935):** **Spec** (`VENDOR_SPECS_DIGEST.md`) — elongated octagon, 18'7"×33'11" (18.583×33.917 ft), with corner cuts; digest confirms this as spec-aligned. **Field observation (to verify, not established fact)** — the real structure's footprint may read as closer to a cut-corner rectangle than the rendered elongated octagon. Review task: reconcile rendered shape against **both** the spec and physical reality where they differ — not to assume the spec is wrong. Scope (full audit vs targeted fixes) and ordering TBD for non-GK20 items.
