...
**Legend:** These `#1`–`#28` numbers are the **session working queue** (2026-08-18 onward). They are **distinct from** Overhaul-scope item numbers (Confirmed/Deferred above) and Immediate-feature-backlog numbers. Where names collide, cross-link explicitly — e.g. queue **#19** ≠ Overhaul deferred item **19**; queue **#8** ≠ Overhaul item **8**; queue **#17** cross-links Snap tuning backlog item **4**.

Complete historical record of `#1`–`#28`. Every row stays permanently with status, commit, and rationale; future changes **append**, never delete.
...
| **3** | **OPEN** | Tooltip audit → hints re-advertisement → Quickstart help conventions. Sequenced **LAST**; bundles with **#18** + version bump; carries the **#12** removal build. |
| **4** | **DONE** `b22d782` | Placed-list two-state snap expand (chevron, 220px ↔ 60%, session-only). Deferred aria-only half resolved by **#25**. |
...
| **24** | **OPEN** | Start Here content pass; can ride **#18**. |
...
| **26** | **OPEN** | Expand-ratio revisit. |
| **27** | **OPEN** | Overlap pill: cycle through flagged rows + relocate under zoom. Pill is **TOP-RIGHT** under layers icon; placement plan must handle readout coexistence. |
...
| Operator docs 1.4.0-dev | (this pass) | README/Quickstart/PROJECT_MAP/Notes + APP_META bump; partial #12 dead-code slice |
