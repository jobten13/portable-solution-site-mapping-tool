...
**Legend:** These `#1`–`#28` numbers are the **session working queue** (2026-08-18 onward). They are **distinct from** Overhaul-scope item numbers (Confirmed/Deferred above) and Immediate-feature-backlog numbers. Where names collide, cross-link explicitly — e.g. queue **#19** ≠ Overhaul deferred item **19**; queue **#8** ≠ Overhaul item **8**; queue **#17** cross-links Snap tuning backlog item **4**. **UPDATE 2026-08-31:** extended to `#35`.

Complete historical record of `#1`–`#28`. Every row stays permanently with status, commit, and rationale; future changes **append**, never delete. **UPDATE 2026-08-31:** extended to `#35`.
...
- **Door-mark move requires exiting Door mode** — as designed.
...
- **Group snap** — rigid-set by outer faces; designable on **#34** true-face work. Workaround (goes in **#3** content when built): snap connector piece first, then rejoin.
