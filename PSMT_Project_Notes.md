...
| **17** | **OPEN** | Intermittent map blur/pan during object drag; no repro; priority rising. **Corresponds to Snap tuning backlog item 4** (cross-link; do not duplicate). Read-only observations on record from the **#28** build rider: `map.dragging.disable`/`enable` unconditional in `start`/`endObjectDrag`; drag ends only via map `mouseup` (no mouseleave/pointercancel/document-level); ambient snap can rewrite pose every `mousemove` near a face; mid-drag `drawObject` tears down/rebuilds layers and rebinds `mousedown` each move; prior theory stands (premature mouseup re-enabling map drag). |
| **18** | **OPEN** | Docs batch at version bump (bundles with **#3**). Accumulated: snap wording; **#11** header/footer erratum; beds-fields README/Quickstart copy; accept-vs-resolve Quickstart line; `PROJECT_MAP` index rows (`getPlacedBounds`/`printFitToPage` removed, `getPlacedObjectsBounds` new; `#snap-to-selected` gone; `latlngToMeters` live); **#12** removal rides here; **#24** can ride here; Ctrl/Cmd snap-suppress discoverability (Quickstart/Start Here mention); Notes general currency pass; **#20** wording confirm. |
...
| **21** | **PARKED** | Multi-select / group move — **RESTORED** on the field scenario of moving a whole laid-out facility within a site; selection-set sizing inventory complete (12 consumer decision points; group drag is new semantics, not a rename); needs its own design pass. Cross-link: Overhaul item **6** / Immediate backlog themes — queue number is the working-queue id for this restore. |
...
- **Ctrl/Cmd snap-suppress discoverability** — feature invisible in UI/docs; fix rides **#18**.
